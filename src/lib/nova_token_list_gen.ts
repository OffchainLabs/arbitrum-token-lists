...
export const novaifyL1List = async (pathOrUrl: string, includeOldDataFields?:boolean) => {
  ...
  const newList = await generateNovaTokenList(l1TokenList, prevArbTokenList, {
...
export const updateNovaifiedList = async (pathOrUrl: string) => {
  ...
  const newList = await generateNovaTokenList(arbTokenList, prevArbTokenList, { 
    includeAllL1Tokens: true
  });
...
export const novaListtoEtherscanList = (
