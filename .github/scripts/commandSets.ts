...
async function addOrbitCommand({
  chainId,
  name,
  path,
  inputList,
...
export async function getOrbitCommands(): Promise<Command[]> {
  const orbitCommands: Command[] = [];

  for (const { name, chainId, parentChainId } of customNetworks) {
